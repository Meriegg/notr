...
export const notesRouter = createTRPCRouter({
	...
	updateNote: protectedProcedure.input(z.object({
		...
		requiresAttention: z.boolean(),
		noteId: z.string()
	})).mutation(async ({ ctx: { prisma, session }, input: { noteId, ...input } }) => {
	...
	importFiles: protectedProcedure.input(z.object({
		...
		files.forEach((file) => {
			...
			const matches = [...file.contents.matchAll(/acc|pass|password|account|email|gmail|mail/gi)]
			if (matches.length >= 1) {
	...
	searchNotes: protectedProcedure.input(z.object({
		query: z.string(),
		tags: z.string().array().optional()
		...
		let finalNotes: { note: Note, contentMatches: RegExpMatchArray[]; titleMatches?: RegExpMatchArray[]; tagMatches?: string[], matchCount: number; }[] = [];
		...
		for (let i = 0; i < notes.length; i++) {
			...
			const queryRegexp = new RegExp(queryWords.join("|").toLowerCase(), "gi");
			console.log(queryRegexp)

			// Find matches inside the title
			const titleMatches = note.title ? [...note.title.matchAll(queryRegexp)] : [];
			// Find matches inside the note's content
			const contentMatches = [...note.content.matchAll(queryRegexp)]

			// Calculate the total number of matches
			const totalNumberOfMatches = titleMatches.length + contentMatches.length;
		...
		const resultsSortedByRanking = finalNotes.sort((a, b) => a.matchCount - b.matchCount)
